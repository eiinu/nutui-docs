...
/** vite */
const modulesDocs = (import.meta as any).glob('/src/docs/react_taro/*.md');
for (const path in modulesDocs) {
  let name = (/docs\/react_taro\/(.*).md/.exec(path) as any[])[1];
  guideRouters.push({
...
const modulesEnDocs = (import.meta as any).glob('/src/docs/react_taro/*.en-US.md');
for (const path in modulesEnDocs) {
  let name = (/docs\/react_taro\/(.*).en-US.md/.exec(path) as any[])[1];
  guideEnRouters.push({
